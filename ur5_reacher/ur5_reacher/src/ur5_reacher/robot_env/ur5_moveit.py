...
register(
        id='UR5MoveItEnv-v0',
        entry_point='ur5_reacher.robot_env.ur5_moveit:UR5MoveItEnv',
        max_episode_steps=10000,
    )

class UR5MoveItEnv(robot_BasicEnv.RobotBasicEnv):
    """
    ...
    def __init__(self):
        """
        Initializes a new UR5 Env environment.
        ...
        * MoveIt! : MoveIt! action server is used to send the joint positions to the robot.
        """
        rospy.loginfo("Starting UR5MoveIt Env")
        ros_gazebo.gazebo_unpause_physics()

        """
        Robot model and controllers parameters
        """
        self.model_name_in_gazebo="ur5"
        self.namespace="/"
        pkg_name="ur_description"
        urdf_file="ur5_robot.urdf.xacro" 
        ...
        """
        if rospy.has_param('/UR5_Reacher/current_robot_num'):
            num_robot = int(rospy.get_param("/UR5_Reacher/current_robot_num") + 1)
        ...
        rospy.set_param('/UR5_Reacher/current_robot_num', num_robot)
        ...
        """
        super(UR5MoveItEnv, self).__init__(   launch_gazebo=False, spawn_robot=True, 
        ...
        ros_launch.ros_launch_from_pkg("ur5_reacher","ur5_moveit.launch")
        ...
        """
        rospy.loginfo("Finished Init of UR5MoveIt Env")
