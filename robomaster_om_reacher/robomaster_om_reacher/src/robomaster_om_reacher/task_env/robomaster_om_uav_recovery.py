...
register(
        id='Robomaster_OM_uav_RecoveryEnv-v0',
        entry_point='robomaster_om_reacher.task_env.robomaster_om_uav_recovery:Robomaster_OM_uav_RecoveryEnv',
        max_episode_steps=10000000
    )

class Robomaster_OM_uav_RecoveryEnv(robot_BasicEnv.RobotBasicEnv):
    """
    ...
    def __init__(self):
        ...
        Describe the task.
        """
        rospy.logwarn("Starting Robomaster_OM_uav_RecoveryEnv Task Env")
        ...
        self.reward_fieldnames = ['timestep', 'total_reward', 'distance_penalty', 'prev_dist_reward', 'goal_reward']
        ...
        super(Robomaster_OM_uav_RecoveryEnv, self).__init__( launch_gazebo=False, spawn_robot=True, gazebo_freq = 10, model_pos_x=self.model_pos_x, model_pos_y=self.model_pos_y, model_pos_z=self.model_pos_z, pkg_name=pkg_name, urdf_file=urdf_file, urdf_folder="/robot",
        ...
        """
        rospy.logwarn("Finished Init of Robomaster_OM_uav_RecoveryEnv Task Env")
    ...
    def _get_reward(self):
        ...
        if done:
            ...
            if self.prev_arrow_scale > self.arrow_scale:
                reward += 50*(self.prev_arrow_scale - self.arrow_scale) # 2/26 .5 -> 1.0 -> 1.5 3/6 2.5
                prev_dist_reward = 50*(self.prev_arrow_scale - self.arrow_scale)
                # rospy.logwarn("Arrow scale decreased -> Goal is closer")
            else:
                reward += 50*(self.prev_arrow_scale - self.arrow_scale)
                prev_dist_reward = 50*(self.prev_arrow_scale - self.arrow_scale)
        ...
        rospy.logwarn(">>>PREV_DIST_REWARD>>>"+str(prev_dist_reward))
