...
register(
        id='Robomaster_OM_MoveItEnv-v0',
        entry_point='robomaster_om_reacher.robot_env.robomaster_om_moveit:Robomaster_OM_MoveItEnv',
        max_episode_steps=10000,
    )

class Robomaster_OM_MoveItEnv(robot_BasicEnv.RobotBasicEnv):
    """
    Superclass for all Robomaster_OM environments.
    """

    def __init__(self):
        """
        Initializes a new Robomaster_OM Env environment.
        ...
        * MoveIt! : MoveIt! action server is used to send the joint positions to the robot.
        """
        rospy.loginfo("Starting Robomaster_OM_MoveIt Env")
        ros_gazebo.gazebo_unpause_physics()

        """
        Robot model and controllers parameters
        """
        self.model_name_in_gazebo="Robomaster_OM"
        self.namespace="/"
        pkg_name="robomaster_om_description"
        urdf_file="robot_description.urdf.xacro" 
        model_pos_x=0.0; model_pos_y=0.3; model_pos_z=0.247
        controller_file="controller.yaml"
        self.controller_list=["joint_state_controller","arm_controller"]

        """
        Use parameters to allow multiple robots in the environment.
        """
        if rospy.has_param('/Robomaster_OM_Reacher/current_robot_num'):
            num_robot = int(rospy.get_param("/Robomaster_OM_Reacher/current_robot_num") + 1)
        ...
        rospy.set_param('/Robomaster_OM_Reacher/current_robot_num', num_robot)
        ...
        """
        super(Robomaster_OM_MoveItEnv, self).__init__(   launch_gazebo=False, spawn_robot=True, 
                    model_name_in_gazebo=self.model_name_in_gazebo, namespace=self.namespace, pkg_name=pkg_name, 
                    urdf_file=urdf_file, controller_file=controller_file, controller_list=self.controller_list,
        ...
        if self.namespace is not None and self.namespace != '/':
            ...
            self.joint_state_topic = "/open_manipulator/joint_states"
        ...
        ros_launch.ros_launch_from_pkg("robomaster_om_reacher","robomaster_om_moveit.launch")
        rospy.logwarn("Waiting for MoveIt to start")
        rospy.wait_for_service("/move_group/trajectory_execution/set_parameters")
        rospy.logwarn("MoveIt started")
        ...
        #--- Start MoveIt Object
        self.move_robomaster_om_object = MoveRobomaster_OM()

        """
        Finished __init__ method
        """
        rospy.loginfo("Finished Init of Robomaster_OM_MoveIt Env")
    ...
    #--------------------------------------------#
    #  Methods created for the UR5 Robot  #
    ...
    def set_trajectory_ee(self, action):
        ...
        result = self.move_robomaster_om_object.ee_traj(ee_target) # TODO -> 바꾸기
        return result
    ...
    def set_trajectory_joints(self, q_positions):
        ...
        """
        result = self.move_robomaster_om_object.joint_traj(q_positions)
    ...
    def get_ee_pose(self):
        ...
        """
        ros_gazebo.gazebo_unpause_physics()
        gripper_pose = self.move_robomaster_om_object.ee_pose() # TODO -> 바꾸기
        ros_gazebo.gazebo_pause_physics()
        return gripper_pose
    ...
    def get_ee_rpy(self):
        ...
        """
        gripper_rpy = self.move_robomaster_om_object.ee_rpy() # TODO -> 바꾸기
        return gripper_rpy

    def get_joint_angles(self):
        ros_gazebo.gazebo_unpause_physics()
        joint_angles = self.move_robomaster_om_object.joint_angles() # TODO -> 바꾸기
        ros_gazebo.gazebo_pause_physics()
        return joint_angles
    ...
    def check_goal(self, goal):
        ...
        * goal is a list with 3 elements, XYZ positions of the EE
        """
        result = self.move_robomaster_om_object.is_goal_reachable(goal) # TODO -> 바꾸기
        return result
    
    def get_randomJointVals(self):
        return self.move_robomaster_om_object.group.get_random_joint_values() # TODO -> 바꾸기

    def get_randomPose(self):
        return self.move_robomaster_om_object.group.get_random_pose() # TODO -> 바꾸기

# Class to move robomaster_om robot through MoveIt

class MoveRobomaster_OM(object):

    def __init__(self):
        rospy.logwarn("===== In MoveRobomaster_OM")
        ...
        self.group_name = "open_manipulator"
        self.group = moveit_commander.MoveGroupCommander(self.group_name)

        rospy.logwarn("===== Out MoveRobomaster_OM")
    ...
    def joint_traj(self, positions_array): # joint traj 
