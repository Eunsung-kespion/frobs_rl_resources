...
from robomaster_om_reacher.task_env.robomaster_om_uav_recovery import Robomaster_OM_uav_RecoveryEnv
...
if __name__ == '__main__':
    ...
    ros_gazebo.launch_Gazebo(paused=True, gui=True, use_sim_time=True)
    ...
    # Start node
    rospy.logwarn("Start")
    rospy.init_node('train_robomaster_om_uav_Recovery')

    # Launch the task environment
    env = gym.make('Robomaster_OM_uav_RecoveryEnv-v0')
    ...
    env = TimeLimitWrapper(env, max_steps=300)
    env.reset()
    ...
    #--- PPO
    # save_path = pkg_path + "/models/static_reacher/ppo/uav_Recovery/"
    # log_path = pkg_path + "/logs/static_reacher/ppo/uav_Recovery/"
    # model = PPO(env, save_path, log_path, config_file_pkg="robomaster_om_reacher", config_filename="ppo.yaml")
